...
router.post('/details', verifyFirebaseToken, async (req, res) => {
  try {
    const { 
      uid, 
      ipAddress, 
    ...
    if (user) {
      ...
      if (googleData) {
        user.displayName = googleData.displayName || user.displayName;
        user.photoURL = googleData.photoURL || user.photoURL;
      }
