...
// Development mode mock data for localhost testing
const DEVELOPMENT_MOCK_DATA = {
  ip: '127.0.0.1',
  city: 'Ibadan',
  region: 'Oyo State',
  country: 'Nigeria',
...
const isDevelopmentMode = (ip) => {
  const localIps = ['::1', '127.0.0.1', 'localhost', '::ffff:127.0.0.1'];
  return localIps.includes(ip) || ip?.startsWith('192.168.') || ip?.startsWith('10.') || ip?.startsWith('172.16.');
};

// Helper function to fetch IP info from multiple services
const fetchLocationFromServices = async (clientIp = null) => {
  // If we're in development mode, return mock data
  if (isDevelopmentMode(clientIp)) {
    console.log('Development mode detected, using mock location data for IP:', clientIp);
    return {
      ...DEVELOPMENT_MOCK_DATA,
      ip: clientIp,
      note: 'Mock data for development - localhost IP detected'
    };
  }
...
router.get('/test', async (req, res) => {
  try {
    res.status(200).json({
      ...
      isDevelopment: isDevelopmentMode(req.ipAddress),
      mockData: isDevelopmentMode(req.ipAddress) ? DEVELOPMENT_MOCK_DATA : null,
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    res.status(500).json({
