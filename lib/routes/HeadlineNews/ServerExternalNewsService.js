...
// Cleaned ServerExternalNewsService.js - Simplified and optimized
import fetch from 'node-fetch'; 
import dotenv from 'dotenv';
import path from 'path';
import crypto from 'crypto';
...
const makeAPICall = async (ipInfo) => {
  ...
  throw new Error('All fallback IPs failed');
};
...
// MAIN FUNCTION: Fetch external news
export const fetchExternalNewsServer = async (ipInfo = { ip: '8.8.8.8' }) => {
  const startTime = Date.now();
