...
// ServerExternalNewsService.js - Fixed version with better error handling and duplicate detection
import fetch from 'node-fetch'; 
import dotenv from 'dotenv';
import path from 'path';
import { fileURLToPath } from 'url';
...
const fetchWithRetry = async (url, options = {}, maxRetries = 3) => {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (response.status === 429) {
        const retryAfter = response.headers.get('retry-after') || (attempt * 60); // Default: 60s, 120s, 180s
      ...
      if (error.message.includes('Rate limited')) {
        throw error; // Don't retry rate limit errors with normal backoff
      }
      
      if (attempt < maxRetries) {
        const waitTime = Math.min(attempt * 5000, 30000); // 5s, 10s, 15s max 30s
        console.log(`⏳ Waiting ${waitTime}ms before retry...`);
        await new Promise(resolve => setTimeout(resolve, waitTime));
      } else {
        throw error;
...
// FIXED: More lenient unique ID generation
const createUniqueId = (url, title) => {
  const normalizeUrl = (url) => {
    try {
      const urlObj = new URL(url);
      // Only remove common tracking parameters, keep the core URL structure
  ...
  // Use a simpler title normalization
  const titleSlug = title ? title.toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, ' ').trim().substring(0, 30) : '';
  
  // Create a more unique combination
  const combinedString = `${normalizedUrl}-${titleSlug}-${Date.now()}`;
  ...
  const uniqueId = `ext-${Math.abs(hash).toString(36)}-${Date.now().toString(36)}`;
  return uniqueId;
};
...
// FIXED: Better duplicate detection and error handling
const processArticlesBatch = async (articles, batchSize = 3) => {
  ...
  for (let i = 0; i < articles.length; i += batchSize) {
    ...
    // FIXED: Process articles sequentially within batch to avoid database conflicts
    for (let j = 0; j < batch.length; j++) {
      const article = batch[j];
      const globalIndex = i + j;
      
      try {
        console.log(`\n🔍 Processing article ${globalIndex + 1}/${articles.length}: "${article.title?.substring(0, 50)}..."`);
        
        // Enhanced validation
        if (!article || typeof article !== 'object') {
          console.log(`❌ Article ${globalIndex + 1}: Invalid object`);
          results.push({ success: false, reason: 'invalid_object' });
          continue;
        }

        if (!article.title?.trim() || !article.url?.trim()) {
          console.log(`❌ Article ${globalIndex + 1}: Missing essential fields (title: ${!!article.title}, url: ${!!article.url})`);
          results.push({ success: false, reason: 'missing_essential_fields' });
          continue;
        }

        // FIXED: Less aggressive duplicate detection
        const uniqueId = createUniqueId(article.url, article.title);
        const normalizedUrl = article.url.toLowerCase().trim();
        
        // Create a more flexible title check
        const titleWords = article.title.toLowerCase().split(' ').filter(word => word.length > 3).slice(0, 5);
        const titlePattern = titleWords.join('.*');
        
        console.log(`🔍 Article ${globalIndex + 1}: Checking duplicates...`);
        console.log(`   - Unique ID: ${uniqueId}`);
        console.log(`   - URL: ${normalizedUrl.substring(0, 50)}...`);
        console.log(`   - Title pattern: ${titlePattern}`);
        
        // SIMPLIFIED duplicate check - only check URL and recent similar titles
        const existingContent = await Content.findOne({ 
          $or: [
            ...
            { 
              $and: [
                { source: 'external' },
                { message: { $regex: new RegExp(titlePattern, 'i') } },
                { createdAt: { $gte: new Date(Date.now() - 6 * 60 * 60 * 1000) } } // Within 6 hours only
              ]
            }
          ]
        });
        ...
        if (existingContent) {
          console.log(`⚠️ Article ${globalIndex + 1} duplicate detected by: ${existingContent.originalUrl === normalizedUrl ? 'URL' : 'title pattern'}`);
          results.push({ success: false, reason: 'exists_in_database', existingId: existingContent._id });
          continue;
        }
        
        // Get or create channel
        const sourceName = article.source?.name || article.source || 'External News';
        console.log(`📺 Article ${globalIndex + 1}: Getting channel for "${sourceName}"`);
        
        const channel = await createOrGetChannelForExternalSource(sourceName);
        
        if (!channel?._id) {
          console.log(`❌ Article ${globalIndex + 1}: Failed to get/create channel`);
        ...
        console.log(`💾 Article ${globalIndex + 1}: Creating content object...`);
        console.log(`   - Category: ${isJustIn ? 'Just In' : 'Headlines'}`);
        ...
        console.log(`💾 Article ${globalIndex + 1}: Saving to database...`);
        
        const savedContent = await newsContent.save();
        const destination = isJustIn ? 'Just In' : 'Headlines';
        console.log(`✅ Article ${globalIndex + 1} saved successfully to ${destination}: "${article.title?.substring(0, 40)}..." from ${sourceName}`);
        
        results.push({ success: true, content: savedContent, uniqueId });
        
      } catch (error) {
        console.error(`❌ Article ${globalIndex + 1} processing error:`, {
        ...
        if (error.code === 11000) {
          ...
          results.push({ success: false, reason: 'database_constraint_duplicate' });
        } else {
