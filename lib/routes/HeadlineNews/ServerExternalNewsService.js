...
// Wake up partner server
const wakeUpPartnerServer = async () => {
  if (!PARTNER_API_URL) {
    console.log('❌ No partner API URL configured');
    return false;
  }
  ...
  if (partnerState.wakeupInProgress) {
    console.log('⏳ Partner wake-up already in progress...');
    return false;
  }

  // Check if we recently tried to wake up (within last 2 minutes)
  if (now - partnerState.lastWakeAttempt < 120000) {
    console.log('⏭️ Recent wake-up attempt, skipping...');
    return partnerState.isAwake;
  }

  try {
    partnerState.wakeupInProgress = true;
    partnerState.lastWakeAttempt = now;
    
    console.log('🔔 Attempting to wake up partner server...');
    
    // Extract base URL for wake-up ping
    const baseUrl = PARTNER_API_URL.replace('/api/news/local', '');
    const wakeupUrl = `${baseUrl}/health`; // or just baseUrl if no health endpoint
    ...
    if (response.ok || response.status < 500) {
      ...
      console.log(`⚠️ Partner server responded with ${response.status}`);
    ...
    partnerState.wakeupInProgress = false;
  }
};
