// ServerExternalNewsService.js - Integrated version with wake-up functionality and comprehensive debugging
...
const PARTNER_API_URL = process.env.PARTNER_API_URL?.trim(); // Clean any whitespace
...
// Wake up partner API function with extended timeout for Render.com
const wakeUpPartnerAPI = async () => {
  try {
    console.log('🔄 Waking up partner API...');
    
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 45000); // 45 seconds for render.com
    ...
    console.log('📡 Partner API wake-up response:', response.status);
    console.log('📡 Partner API response headers:', Object.fromEntries(response.headers));
    
    if (!response.ok) {
      const errorText = await response.text();
      console.error('❌ Partner API wake-up error response:', errorText);
      throw new Error(`Partner API wake-up failed: ${response.status} ${response.statusText} - ${errorText}`);
    }

    const data = await response.json();
    console.log('📰 Wake-up API Response structure:', {
    ...
    console.error('❌ Partner API wake-up failed:', error);
    if (error.name === 'AbortError') {
      console.error('⏰ Wake-up request timed out after 45 seconds');
    }
    throw error;
  }
};

// Process articles in batches with comprehensive error handling
const processArticlesBatch = async (articles, batchSize = 3) => {
  ...
  for (let i = 0; i < articles.length; i += batchSize) {
    ...
    // Longer delay between batches for Render.com stability
...
// MAIN INTEGRATED FUNCTION with wake-up functionality and comprehensive debugging
export const fetchExternalNewsServer = async (ipInfo = { ip: '8.8.8.8' }) => {
  try {
    ...
    // STEP 1: Wake up the partner API first
    console.log('🔄 Step 1: Waking up partner API...');
    let data;
    try {
      data = await wakeUpPartnerAPI();
    } catch (error) {
      console.error('❌ Partner API wake-up failed, retrying in 5 seconds...');
      await new Promise(resolve => setTimeout(resolve, 5000));
      try {
        data = await wakeUpPartnerAPI();
      } catch (retryError) {
        console.error('❌ Partner API wake-up failed on retry, attempting direct fetch...');
        
        // Fallback to direct fetch with IP parameter
        const fullUrl = `${PARTNER_API_URL}?ip=${encodeURIComponent(ipInfo.ip)}`;
        console.log('🌐 [SERVER] Fallback Full API URL:', fullUrl);
        ...
        if (!response.ok) {
          const errorText = await response.text();
          throw new Error(`Fallback API request failed: ${response.status} ${response.statusText} - ${errorText}`);
        }
    ...
    if (!data.articles || !Array.isArray(data.articles)) {
      ...
      console.log('📋 [SERVER] Full response sample:', JSON.stringify(data, null, 2).substring(0, 500));
      return [];
    }
...
export const testPartnerAPI = async (ipInfo = { ip: '8.8.8.8' }) => {
  try {
    ...
    const response = await fetch(fullUrl, {
      ...
      headers: {
        'Content-Type': 'application/json',
        'User-Agent': 'TruePaceNewsApp-Test/1.0',
        'Accept': 'application/json',
      },
      signal: controller.signal
    });
...
// Create API routes for manual triggering and testing
