...
// ServerExternalNewsService.js - Cold start optimized for free hosting
import fetch from 'node-fetch'; 
import crypto from 'crypto';
import { Content, Channel } from '../../models/HeadlineNews/HeadlineModel.js';

// Cold start aware fetch with partner server wake-up
const fetchWithColdStartHandling = async (url, options = {}) => {
  const startTime = Date.now();
  
  try {
    // STEP 1: Wake up partner server (since it's also on free Render)
    console.log('☕ Waking up partner server...');
    const partnerBaseUrl = new URL(url).origin;
    
    try {
      const wakeUpResponse = await fetch(`${partnerBaseUrl}/`, { 
        method: 'GET',
        timeout: 15000 // 15 seconds for wake-up
      });
      console.log(`✅ Partner server awake: ${wakeUpResponse.status}`);
      
      // Give server time to fully initialize after cold start
      await new Promise(resolve => setTimeout(resolve, 2000));
    } catch (wakeError) {
      console.log('⚠️ Wake-up call failed, continuing anyway...');
    }
    
    // STEP 2: Make actual API request with reduced timeout
    console.log('📡 Making main API request...');
    const controller = new AbortController();
    
    // Reduced to 20 seconds (under cron-job.org's 30s limit)
    const timeoutId = setTimeout(() => {
    ...
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'Accept': 'application/json',
        'Cache-Control': 'no-cache',
        'X-Client-IP': options.clientIp || '8.8.8.8',
        ...options.headers
      }
    });
...
const createOrGetChannelForExternalSource = async (source) => {
  try {
    const normalizedSourceName = source.trim();
    ...
    const savedChannel = await channel.save();
    console.log(`✅ Created channel: ${normalizedSourceName}`);
    return savedChannel;
...
// Generate stable hash for duplicate detection
const createStableHash = (url, title) => {
  const normalizedUrl = url.toLowerCase()
    .replace(/^https?:\/\/(www\.)?/, '')
    .replace(/[?#].*$/, '')
...
// MAIN FETCH FUNCTION - Optimized for cold starts
export const fetchExternalNewsServer = async (ipInfo = { ip: '8.8.8.8' }) => {
  const startTime = Date.now();
  
  try {
    console.log('\n🚀 ============ FETCHING EXTERNAL NEWS ============');
    console.log(`🔍 Using IP: ${ipInfo.ip}`);
    
    if (!process.env.PARTNER_API_URL) {
      throw new Error('PARTNER_API_URL not configured');
    }
    
    // Build API URL
    const url = new URL(process.env.PARTNER_API_URL);
    url.searchParams.set('ip', ipInfo.ip);
    url.searchParams.set('timestamp', Date.now().toString());
    
    // Make request with cold start handling
    const response = await fetchWithColdStartHandling(url.toString(), { 
      method: 'GET',
      clientIp: ipInfo.ip
    });
    ...
    if (!data.articles?.length) {
      console.log('ℹ️ No articles received');
      return [];
    }
    ...
    // Process articles
    const results = [];
    const maxArticles = 30; // Reasonable limit for free hosting
    ...
    console.log(`📝 Processing ${validArticles.length} articles...`);
    ...
    for (const article of validArticles) {
      try {
        ...
        // 48 hours max age for external news
        const maxAge = 48 * 60 * 60 * 1000;
        if (timeSincePublished > maxAge) {
          continue; // Skip old articles
        }
        
        // Just In logic: 15 minutes
        const justInDuration = 15 * 60 * 1000;
        const isJustIn = timeSincePublished < justInDuration;
        const justInExpiresAt = isJustIn ? new Date(publishedAt.getTime() + justInDuration) : null;
        const headlineExpiresAt = new Date(publishedAt.getTime() + maxAge);
    ...
    const duration = Date.now() - startTime;
    console.log(`✅ Saved ${results.length}/${validArticles.length} articles in ${duration}ms`);
    console.log(`🏷️ Just In: ${results.filter(r => r.isJustIn).length}, Headlines: ${results.filter(r => !r.isJustIn).length}`);
