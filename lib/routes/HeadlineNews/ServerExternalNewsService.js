    ...
    const batchPromises = batch.map(async (article, index) => {
      try {
        ...
        if (processedUrls.has(normalizedUrl) || processedTitles.has(normalizedTitle)) {
          console.log(`⚠️ Skipping article ${globalIndex + 1}: Duplicate in current batch`);
          return { success: false, reason: 'duplicate_in_batch' };
        }
        ...
        const existingContent = await Content.findOne({ 
          $or: [
            ...
            { message: { $regex: `^${article.title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}` } }
          ]
        });
        
        if (existingContent) {
          console.log(`⚠️ Article ${globalIndex + 1} already exists: "${article.title?.substring(0, 30)}..."`);
