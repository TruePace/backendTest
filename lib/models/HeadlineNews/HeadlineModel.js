...
const ContentSchema = new mongoose.Schema({
  ...
  // Enhanced externalId with proper uniqueness
  externalId: {
    type: String,
    sparse: true, // Only applies uniqueness to non-null values
    index: true
  }
});

// Compound unique index for external content
ContentSchema.index(
  { externalId: 1 },
  {
    unique: true,
...
// 2. Add index for title-based deduplication (first 100 chars)
ContentSchema.index(
  { "message": "text", source: 1 },
  { 
    partialFilterExpression: { source: 'external' }
  }
);

// 3. CRITICAL: Cleanup function to run immediately
const cleanupDuplicatesNow = async () => {
  try {
    console.log('🧹 Emergency duplicate cleanup...');
    ...
    console.log(`🗑️ Removed ${totalRemoved} duplicate articles`);
    return totalRemoved;
