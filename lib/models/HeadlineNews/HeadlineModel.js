...
const ContentSchema = new mongoose.Schema({
  ...
  fetchedAt: { type: Date }, // When the external content was fetched
  // Add URL hash for faster duplicate detection
  urlHash: { type: String, index: true },
  // Add title hash for content similarity
  titleHash: { type: String, index: true }
});

// CRITICAL: Unique indexes for duplicate prevention
ContentSchema.index(
  { externalId: 1 },
  {
    unique: true,
...
// Compound index for URL-based duplicate detection
ContentSchema.index(
  { originalUrl: 1, source: 1 },
  {
    unique: true,
...
// Index for title-based similarity detection
ContentSchema.index({ titleHash: 1, source: 1 });

// Index for URL hash-based detection
ContentSchema.index({ urlHash: 1, source: 1 });
...
ContentSchema.index({ channelId: 1, createdAt: -1 });
ContentSchema.index({ isJustIn: 1, justInExpiresAt: 1 });

// Pre-save middleware to generate hashes
ContentSchema.pre('save', function(next) {
  if (this.originalUrl && !this.urlHash) {
   this.urlHash = crypto
  .createHash('md5')
  .update(this.originalUrl.toLowerCase().replace(/[?#].*$/, ''))
  .digest('hex');
  }
  
  if (this.message && !this.titleHash) {
    const titleForHash = this.message.split('\n')[0] // Get first line (title)
      .toLowerCase()
      .replace(/[^\w\s]/g, '')
      .trim();
