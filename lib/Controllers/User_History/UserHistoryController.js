...
export const getUserHistory = async (req, res) => {
  try {
    ...
    // Ensure we're sorting by viewedAt in descending order
    const history = await UserHistory.find({ userId: uid })
      .sort({ viewedAt: -1 }) // This will show most recently viewed items first
      .skip(skip)
      .limit(limit)
      .lean();
...
export const addToHistory = async (userId, contentId, contentType, duration) => {
  try {
    // Always update viewedAt when content is viewed
...
export const addToHandler = async (req, res) => {
  try {
    const { uid } = req.user;
