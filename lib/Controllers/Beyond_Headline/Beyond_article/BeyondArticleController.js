...
export const viewArticle = async (req, res) => {
  try {
    ...
    // Increment viewsCount every time
    ...
    const interaction = new ArticleInteraction({
      ...
      deviceInfo: req.headers['user-agent'],
      location: req.ip // You might want to use a geolocation service for more accurate location data
    });
...
export const updateReadTime = async (req, res) => {
  try {
    ...
    const { articleId } = req.params;
    const { readTime, location } = req.body;
    ...
    // Update the ArticleInteraction
    await ArticleInteraction.findOneAndUpdate(
      { userId: uid, articleId: article._id, interactionType: 'view' },
      { 
        $set: { 
          duration: readTime,
          location: {
    ...
    // Update avg read time
    const totalReadTime = article.avgReadTime * article.uniqueViewersCount + readTime;
    article.avgReadTime = totalReadTime / (article.uniqueViewersCount + 1);

    // Update engagement score
    article.engagementScore = article.calculateEngagementScore();

    // Update viral score
    article.viralScore = article.calculateViralScore();

    await article.save();

    res.json({ message: "Read time and location updated successfully" });
  } catch (error) {
    console.error('Error in updateReadTime:', error);
    res.status(500).json({ message: error.message });
  }
};
