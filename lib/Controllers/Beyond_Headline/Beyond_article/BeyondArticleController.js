...
export const createArticle = async (req, res) => {
  try {
    const { title, previewContent, previewImage, fullContent, channelId, readTime,tags } = req.body;
...
export const likeArticle = async (req, res) => {
  try {
    ...
    const isLiking = likeIndex === -1; // true if user is liking, false if unliking
    ...
    if (isLiking) {
      ...
      await ArticleInteraction.deleteOne({
        userId: uid,
        articleId: article._id,
        interactionType: 'like'
      });
...
export const viewArticle = async (req, res) => {
  try {
      ...
      // Only process views with significant duration
      if (duration >= 60) { // 30 seconds minimum
          ...
          if (!recentView) {
              // Increment view count only for new significant engagements
...
export const shareArticle = async (req, res) => {
  try {
      const { articleId } = req.params;
      const { uid } = req.user;
      const { platform, location } = req.body; // location might be null
