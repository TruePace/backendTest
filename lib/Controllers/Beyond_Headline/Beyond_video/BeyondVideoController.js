...
import { User } from "../../../models/HeadlineNews/HeadlineModel.js";
 ...
 export const getAllVideos = async (req, res) => {
  try {
    ...
    console.log('Videos found:', videos.length); // Log the number of videos found

    res.status(200).json(videos);
  } catch (error) {
    console.error('Error in getAllVideos:', error); // Log the full error
...
export const postBeyondVideoComment = async (req, res) => {
  ...
  export const viewVideo = async (req, res) => {
    try {
      ...
      await addToHistory(uid, videoId, 'video', watchDuration);
