...
const removeDuplicateExternalContent = async (req, res) => {
  try {
    ...
    let removedCount = 0;
    for (const group of duplicatesByExternalId) {
      // Keep the first one, remove the rest
      const toRemove = group.docs.slice(1);
      await Content.deleteMany({ _id: { $in: toRemove } });
      removedCount += toRemove.length;
    }
    
    // Find duplicates by originalUrl
    const duplicatesByUrl = await Content.aggregate([
      { $match: { source: 'external', originalUrl: { $exists: true, $ne: null } } },
      { $group: { _id: '$originalUrl', count: { $sum: 1 }, docs: { $push: '$_id' } } },
      { $match: { count: { $gt: 1 } } }
    ]);
    
    for (const group of duplicatesByUrl) {
      const toRemove = group.docs.slice(1);
      await Content.deleteMany({ _id: { $in: toRemove } });
      removedCount += toRemove.length;
    }
    ...
    res.json({ removedCount, message: `Successfully removed ${removedCount} duplicates` });
